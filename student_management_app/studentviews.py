...
from student_management_app.models import Student, Courses, Subject, CustomUser, Attendance, AttendanceReport, \
    LeaveReportStudent, FeedBackStudent, NotificationStudent, SessionYearModel

# , StudentResults, OnlineClassRooms,
def student_home(request):
...
#             check_course=Student.objects.filter(admin=request.user.id,course_id=course.id)
#             if check_course.exists():
#                 session_check=Student.objects.filter(admin=request.user.id,session_year_id=session_year_obj.id)
...
def student_feedback(request):
    ...
    return render(request, "hod_template/student_feedback.html", {"feedback_data":feedback_data})
