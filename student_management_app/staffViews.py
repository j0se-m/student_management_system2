...
from student_management_app.models import Subject, SessionYearModel, Student, Attendance, AttendanceReport, \
    LeaveReportStaffs, Staffs, FeedBackStaffs, CustomUser, Courses, NotificationStaffs


# StudentResults, OnlineClassRoom
def staff_home(request):
...
def staff_apply_leave_save(request):
    if request.method != "POST":
        ...
        staff_obj = Staffs.objects.get(admin=request.user.id)
        try:
            leave_report = LeaveReportStaffs(staff_id=staff_obj, leave_date=leave_date, leave_message=leave_msg,
                                             leave_status=0)
            leave_report.save()
...
#
#     try:
#         check_exist=StudentResul.objects.filter(subject_id=subject_obj,student_id=student_obj).exists()
#         if check_exist:
